...
export default function Authenticated({
    ...
    const { user, notifications, unreadNotificationsCount } = usePage().props.auth as any;
        ...
        <div className="min-h-screen bg-gray-100 flex">
            ...
            <aside className="w-64 bg-gray-900 text-white min-h-screen hidden md:flex flex-col flex-shrink-0">
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    ...
                    {/* Flash Sales Link */}
                    <Link
                        href={route('admin.flash_sales.index')}
                        className={`block px-4 py-2 rounded transition ${route().current('admin.flash_sales.*') ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'
                            }`}
                    >
                        Flash Sales
                    </Link>
                    ...
                    <Link
                        ...
                        className={`block px-4 py-2 rounded transition ${route().current('admin.coupons.*') ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'
                            }`}
                    >
                        Coupons
                    </Link>
                    ...
                    <Link
                        ...
                        className={`block px-4 py-2 rounded transition ${route().current('wishlist.index') ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'
                            }`}
                    >
                        My Wishlist
                    </Link>
                                ...
                                {notifications && notifications.length > 0 ? (
                                    ...
                                    notifications.map((notification) => (
                                        <Link
                                            ...
                                            className={`block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 transition border-b border-gray-50 ${!notification.read_at ? 'bg-indigo-50' : ''}`}
                                        >
                                            <p className="font-medium text-indigo-600">{notification.data.message}</p>
